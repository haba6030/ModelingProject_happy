...
stan_data = {
    ...
    "i_vec": i_vec,
    "t_vec": t_vec,
    "bdi_score": bdiScore.tolist() 
}

with open("bldata_bdi.json", "w") as f:
...
def predict_happiness(params, Winlose, chose_risky, choice, bdiScore, rated_mask):
    ...
    # 순차 업데이트
    for t in range(1, T):
        w_phat = mu_phat + bdi_phat * bdiScore
        w_PPE = mu_PPE + bdi_PPE * bdiScore
...
def nll_rated(params, Winlose, chose_risky, choice, bdiScore, rated_mask, H_obs):
    ...
    mu = predict_happiness(params, Winlose, chose_risky, choice, bdiScore, rated_mask)
...
def fit_mle_per_participant(df_trials, bdiScores, id_x = "id", happiness = "happiness", 
    ...
    for pid, group in df_trials.groupby(id_x):
        ...
        for _ in range(10):  # 10번 반복
            ...
            res = minimize(
                nll_rated, 
                x0=x0, 
                args=(Winlose, chose_risky, choice, bdiScore, rated_mask, H_obs),
        ...
        # 예측값 생성
        logL = -best_loss
        mu_full = predict_happiness(best_params, Winlose, chose_risky, choice, 
                                    bdiScore, rated_mask)
        y_true = H_obs
    ...
    df_result = pd.DataFrame(results, 
                             columns=["id", "w0", "mu_Phat", "bdi_Phat",  
                                      "mu_PPE", "bdi_Phat", "gamma", "alpha"])
...
# # 결과 저장
df_params.to_csv("mle_bdionly_results.csv", index=False)
