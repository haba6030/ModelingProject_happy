...
transformed parameters {
  ...
  w4 = mu_pr[5] + sigma[5] * w4_pr;

  for (i in 1:N) {gam[i] = Phi_approx(mu_pr[6] + sigma[6] * gam_pr[i]);}
  for (i in 1:N) {alpha[i] = inv_logit(mu_pr[7] + sigma[7] * alp_pr[i]);}  
}
...
model {
  ...
  w4_pr ~ student_t(3, 0, 1);      // outlier 포함 가능
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (i == i_vec[count] && t == t_vec[count]) {
        real mu = w0[i] + w1[i] * CR_decay + w2[i] * EV_decay + w3[i] * RPE_decay + w4[i] * PPE_decay;
...
generated quantities {
  ...
  mu_w3    = mu_pr[4];
  mu_w4    = mu_pr[5];
  mu_gam   = Phi_approx(mu_pr[6]);
  mu_alp   = Phi_approx(mu_pr[7]);
